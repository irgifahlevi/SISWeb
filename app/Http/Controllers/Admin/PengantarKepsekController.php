...
class PengantarKepsekController extends Controller
{
    ...
    {
        $search_pengantarkepsek = $request->query('search_pengantarKepsek');

        $query = PengantarKepsek::where('row_status', '0')->orderBy('id', 'desc');

        if (!empty($search_pengantarkepsek)) {
            $query->where('title', 'like', '%' . $search_pengantarkepsek . '%');
        }

        $pengantarKepsek = $query->paginate(5)->onEachSide(2)->fragment('PengantarKepsek');

        return view('AdminView.PengantarKepsek.index', compact('pengantarKepsek'));
    }

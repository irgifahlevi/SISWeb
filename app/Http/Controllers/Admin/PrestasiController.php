...
class PrestasiController extends Controller
{
    ...
    {
        $search_prestasi = $request->query('search_prestasi');

        // Mulai dengan kueri dasar untuk mendapatkan semua akun siswa
        $query = KurikulumPrestasi::where('jenis_info', 'prestasi')
            ->where('row_status', '0')
            ->orderBy('id', 'desc');

        // Jika ada pencarian, tambahkan filter pencarian ke kueri
        if (!empty($search_prestasi)) {
            $query->where('nama', 'like', '%' . $search_prestasi . '%');
        }

        // Ambil data dengan paginasi
        $prestasi = $query->paginate(5)->onEachSide(2)->fragment('prestasi_sekolah');

        return view('AdminView.Prestasi.index', compact('prestasi', 'search_prestasi'));
    }
    ...
    public function show(string $id)
    {
        try {
            $galeri = KurikulumPrestasi::where('id', $id)
                ->where('jenis_info', 'prestasi')->where('row_status', '0')->firstOrFail();

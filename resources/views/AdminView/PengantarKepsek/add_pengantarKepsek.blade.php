<div class="modal fade" id="add-pengantarKepsek-modal" tabindex="-1" role="dialog" aria-labelledby="modal-add-pengantarKepsek" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <form id="form-add-pengantarKepsek" enctype="multipart/form-data">
...
@section('scripts')
<script>
    $('body').on('click', '#add-pengantarKepsek', function() {
        // open modal
        $('#loading-overlay').show();
        setTimeout(() => {
            $('#loading-overlay').hide();
            $('#add-pengantarKepsek-modal').modal('show');
        }, 800);
    });
    ...
    $(document).ready(function() {
        $('#form-add-pengantarKepsek').on('submit', function(e){
            e.preventDefault();
        ...
        $.ajax({
            type: 'POST',
            url: '{{route('pengantarKepsek.store')}}',
            data: formData,
            dataType: 'json',
            processData: false,
            contentType: false,
            ...
            if(response.status == 200){
                ...
                $('#loading-overlay').hide();
                $('#add-pengantarKepsek-modal').modal('hide');
                $('#form-add-pengantarKepsek')[0].reset();
    ...
    $('#add-pengantarKepsek-modal').on('hidden.bs.modal', function (e){
        $('.error-message').text(''); //menghapus pesan error
        $('#form-add-pengantarKepsek')[0].reset(); //mereset form
    });

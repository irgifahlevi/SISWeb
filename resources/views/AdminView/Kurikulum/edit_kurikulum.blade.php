<div class="modal fade" id="edit-modal-kurikulum" tabindex="-1" role="dialog" aria-labelledby="modal-edit" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div id="data-container">
        <form id="edit-form-kurikulum" enctype="multipart/form-data">
...
<script>
  ...
  $('body').on('click', `#edit-kurikulum`, function () {
    var id = $(this).data('id'); // menangkap ID dari data attribute 'data-id'
    //id = 11;
    // kosongkan form
    $('#edit-form-kurikulum')[0].reset();
    // tampilkan spinner
    ...
    setTimeout(() => {
      ...
      success: function (response) {
        if(response.status == 200) {
          ...
          $('#edit-modal-kurikulum').modal('show');

          $('#edit-modal-kurikulum #data-container').hide();
          
          // sembunyikan spinner
          $('#loading-overlay').hide();
          
          // tampilkan data pada form
          $('#edit-modal-kurikulum #data-container').show();

          $('#edit-modal-kurikulum').find('input[name="id"]').val(data.id);
          $('#edit-modal-kurikulum').find('input[name="title"]').val(data.title);
          $('#edit-modal-kurikulum').find('input[name="nama"]').val(data.nama);
          $('#edit-modal-kurikulum').find('textarea[name="deskripsi"]').val(data.deskripsi);
          $('#edit-modal-kurikulum').find('input[name="tahun"]').val(data.tahun);
        }
      },
  ...
  $(document).ready(function(){
    $('#edit-form-kurikulum').on('submit', function(e){
      e.preventDefault();
      // console.log('test');
      const id = $('#edit-modal-kurikulum').find('input[name="id"]').val();
      const title = $('#edit-modal-kurikulum').find('input[name="title"]').val();
      const nama = $('#edit-modal-kurikulum').find('input[name="nama"]').val();
      const tahun = $('#edit-modal-kurikulum').find('input[name="tahun"]').val();
      const deskripsi = $('#edit-modal-kurikulum').find('textarea[name="deskripsi"]').val();
      const gambar = $('#edit-modal-kurikulum').find('input[name="gambar"]')[0].files[0];
      ...
      setTimeout(() => {
        $.ajax({
          url: '{{ route('kurikulum-sekolah.update', ':id') }}'.replace(':id', id),
          type: 'POST',
          headers: {
          ...
          success: function(response){
            if(response.status == 200){
                ...
                $('#edit-modal-kurikulum').modal('hide');
                $('#edit-form-kurikulum')[0].reset();
                $('#loading-overlay').hide();
  ...
  $(document).ready(function () {
    ...
    $('#edit-modal-kurikulum').on('hidden.bs.modal', function (e) {
      $('.error-messages').text('');
    });
  });
</script>
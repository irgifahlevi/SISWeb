<div class="modal fade" id="add-modal-kurikulum" tabindex="-1" role="dialog" aria-labelledby="modal-add-kategori" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form id="form-add-kurikulum" enctype="multipart/form-data">
...
@section('scripts')
    <script>
      $('body').on('click', '#add-kurikulum', function () {
        //open modal
        $('#loading-overlay').show();
        setTimeout(() => {
          $('#loading-overlay').hide();
          $('#add-modal-kurikulum').modal('show');
        }, 800);
      });

      // Simpan data
      $(document).ready(function(){
        $('#form-add-kurikulum').on('submit', function(e){
          e.preventDefault();
          // console.log('test');
          ...
          setTimeout(() => {
            $.ajax({
              type: 'POST',
              url: '{{route('kurikulum-sekolah.store')}}',
              data: formData,
              dataType: 'json',
              processData: false,
              contentType: false,
              success: function(response)
              {
                  if(response.status == 200){
                    ...
                    $('#loading-overlay').hide();
                    $('#add-modal-kurikulum').modal('hide');
                    $('#form-add-kurikulum')[0].reset();
      ...
      $(document).ready(function () {
        ...
        $('.close-modal-tambah').on('click', function (e) {
          ...
          $('#form-add-kurikulum')[0].reset(); // Mereset form
        });
        
        // Menambahkan event listener pada modal
        $('#add-modal-kurikulum').on('hidden.bs.modal', function (e) {
          $('.error-message').text(''); // Menghapus pesan error
          $('#form-add-kurikulum')[0].reset(); // Mereset form
        });
      });

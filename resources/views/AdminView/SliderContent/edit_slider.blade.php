<div class="modal fade" id="edit-modal-slider" tabindex="-1" role="dialog" aria-labelledby="modal-edit" aria-hidden="true">
...
<script>
    ...
    setTimeout(() => {
      ...
      success: function (response) {
        if(response.status == 200) {
          ...
          $('#edit-modal-slider').modal('show');

          $('#edit-modal-slider #data-container').hide();
          
          // sembunyikan spinner
          $('#loading-overlay').hide();
          
          // tampilkan data pada form
          $('#edit-modal-slider #data-container').show();

          $('#edit-modal-slider').find('input[name="id"]').val(data.id);
          $('#edit-modal-slider').find('input[name="title"]').val(data.title);
          $('#edit-modal-slider').find('textarea[name="deskripsi"]').val(data.deskripsi);
        }
      },
  ...
  $(document).ready(function(){
    $('#edit-form-slider').on('submit', function(e){
      e.preventDefault();
      // console.log('test');
      const id = $('#edit-modal-slider').find('input[name="id"]').val();
      const title = $('#edit-modal-slider').find('input[name="title"]').val();
      const deskripsi = $('#edit-modal-slider').find('textarea[name="deskripsi"]').val();
      const gambar = $('#edit-modal-slider').find('input[name="gambar"]')[0].files[0];
      ...
      setTimeout(() => {
        $.ajax({
          url: '{{ route('slider-content.update', ':id') }}'.replace(':id', id),
          type: 'POST',
          headers: {
  ...
  $(document).ready(function () {
    ...
    $('#edit-modal-slider').on('hidden.bs.modal', function (e) {
      $('.error-messages').text('');
    });
  });
</script>
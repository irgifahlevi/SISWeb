<div class="modal fade" id="add-sejarah-modal" tabindex="-1" role="dialog" aria-labelledby="modal-add-sejarah" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <form id="form-add-sejarah" enctype="multipart/form-data">
...
@section('scripts')
<script>
    $('body').on('click', '#add-sejarah', function() {
        // open modal
        $('#loading-overlay').show();
        setTimeout(() => {
            $('#loading-overlay').hide();
            $('#add-sejarah-modal').modal('show');
        }, 800);
    });
    ...
    $(document).ready(function() {
        $('#form-add-sejarah').on('submit', function(e){
            e.preventDefault();
        ...
        $.ajax({
            ...
            if(response.status == 200){
                ...
                $('#loading-overlay').hide();
                $('#add-sejarah-modal').modal('hide');
                $('#form-add-sejarah')[0].reset();
    ...
    $('#add-sejarah-modal').on('hidden.bs.modal', function (e){
        $('.error-message').text(''); //menghapus pesan error
        $('#form-add-sejarah')[0].reset(); //mereset form
    });
